...
file_paths = [
    r'D:\Elavenil\Miun\Phase 6\Text files\Thresholdscan\Mo threshold scan.txt',
]

# Dictionary to store grouped data
grouped_data = defaultdict(lambda: {'temperatures': [], 'mu': [], 'sigma': [], 'energy': [], 'mu_20': None, 'sigma_20': None, 'energy_shift': [], 'relative_error': []})

reference_energy = 17.5  # keV for 20°C μ value
    ...
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as file:
        ...
        for line in lines:
            try:
                ...
                # Compute Equivalent Energy for mu & sigma
                ...
                energy_shift = equivalent_energy_mu - reference_energy  # Energy shift relative to 20°C
...
for group_key, temperature, mu_value, sigma_value in pending_entries:
    if grouped_data[group_key]['mu_20'] is not None and grouped_data[group_key]['sigma_20'] is not None:
        ...
        equivalent_energy_mu = (mu_value / mu_20) * reference_energy
        equivalent_energy_sigma = (sigma_value / sigma_20) * reference_energy
        energy_shift = equivalent_energy_mu - reference_energy
...
# Plot 2: Energy Shift (ΔE) vs Ikrum Group
plt.figure(figsize=(12, 6))
for group, data in grouped_data.items():
    group_numeric = ikrum_to_numeric[group]
    for i, temperature in enumerate(data['temperatures']):
        if data['energy_shift'][i] is None:
            continue
        offset = (i - (len(data['temperatures']) - 1) / 2) * 0.2  
        temp_color = colors[unique_temperatures.index(temperature)]
        
        label = f'Temperature: {temperature}°C'
        plt.scatter(group_numeric + offset, data['energy_shift'][i], color=temp_color,label=label, s=80, edgecolors='black')
...
# Print the absolute energy shift, relative error, and energy resolution in a single line
for group_key, data in grouped_data.items():
    for i, temperature in enumerate(data['temperatures']):
        # Only process if we have valid data
        if data['energy_shift'][i] is None or data['relative_error'][i] is None:
            continue
        
        # Calculate sigma in energy terms
        sigma_20 = grouped_data[group_key]['sigma_20']
        if sigma_20 is None or data['energy'][i] == 0:
            continue  # Skip if no valid sigma_20 or energy is zero
        
        # Energy equivalent for sigma
        equivalent_energy_sigma = (data['sigma'][i] / sigma_20) * reference_energy
        
        # Calculate energy resolution using the mean energy for the group
        mean_energy = data['energy'][i]  # Mean energy for the current temperature
        resolution = (equivalent_energy_sigma / mean_energy) * 100  # Percentage resolution
        
        # Print the results: Absolute Shift, Relative Error, and Energy Resolution
        print(f"Group_Mo: {group_key}, Temperature: {temperature}°C, "
              f"Absolute Shift: {data['energy_shift'][i]:.4f} keV, "
              f"Relative Error: {data['relative_error'][i]:.2f}%, "
              f"Energy Resolution = {resolution:.2f}%")

 
plt.title('Mean Energy Shift (Δμ) vs Ikrum Group')
plt.xlabel('Ikrum Group')
plt.ylabel('Shifted Mean Energy (Δμ)')
plt.xticks(ticks=np.arange(1, len(ikrum_groups) + 1), labels=ikrum_groups)
plt.grid(True, linestyle='--', alpha=0.7)
plt.show()

# Plot 3: Relative Error (%) vs Ikrum Group
plt.figure(figsize=(12, 6))
for group, data in grouped_data.items():
    group_numeric = ikrum_to_numeric[group]
    for i, temperature in enumerate(data['temperatures']):
        # Skip entries if the relative error is not available
        if data['relative_error'][i] is None:
            continue
        offset = (i - (len(data['temperatures']) - 1) / 2) * 0.2  
        temp_color = colors[unique_temperatures.index(temperature)]
        
        label = f'Temperature: {temperature}°C'
        plt.scatter(group_numeric + offset, data['relative_error'][i], color=temp_color, label=label, s=80, edgecolors='black')
...
plt.title('Relative Error (%) vs Ikrum Group')
plt.xlabel('Ikrum Group')
